...
internal data class DockerDebugModeAzureInfo(
    ...
    val azureFunctionsToolsPathInContainer: String,
    val azureFunctionsJsonOutputFileInContainer: String,
    val azureFunctionsJsonOutputFileInHost: String,
)
...
internal fun DockerDebugModeAzureInfo.getDebugModeVolumes() = buildList {
    ...
    add(debuggerVolumes.azureFunctionsToolsFolder.toVolumeBinding())
    add(debuggerVolumes.azureFunctionsArtifactsFolder.toVolumeBinding())
}
...
private const val BASE_FUNC_START_ARGS =
    "start --dotnet-isolated-debug --enable-json-output --json-output-file"

internal fun DockerDebugModeAzureInfo.getEntrypoint(containerOs: DockerContainerType): List<String> {
    ...
    val funcCommand =
        "$azureFunctionsToolsPathInContainer $BASE_FUNC_START_ARGS $azureFunctionsJsonOutputFileInContainer"

    val workerCommand =
        "${commandLine.exePath} ${commandLine.parametersList.list.joinToString(" ")}"

    return if (!isWindowsContainer) {
        val fullCommand = "$funcCommand & exec $workerCommand"
        listOf(
            "sh",
            "-c",
            fullCommand
        ...
        val fullCommand =
            "Start-Process -FilePath \"$azureFunctionsToolsPathInContainer\" -ArgumentList \"$BASE_FUNC_START_ARGS $azureFunctionsJsonOutputFileInContainer\"; $workerCommand"
        listOf(
            "powershell",
            "-NoLogo",
            "-NoProfile",
...
internal fun DockerDebugModeAzureInfo.getExecutionCommand(): List<String> {
    return listOf(azureFunctionsJsonOutputFileInHost)
}